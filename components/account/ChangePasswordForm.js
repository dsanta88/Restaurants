...
export default function ChangePasswordForm({setShowModal,toastRef}) {
    ...
    if(!resultUpdatePassword.statusResponse){
        setErrorNewPassword("Hubo un error cambiando la contraseña. Intente mas tarde.")
        return
     }
    ...
    const validateForm=()=>{
      ...
      if(size(newPassword)<6){
          setErrorNewPassword("Dedes ingresas una contraseña de al menos 6 caracteres.")
          isValid= false
      }

      
      if(size(confirmPassword)<6){
        setErrorConfirmPassword("Dedes ingresas una confirmación de tu contraseña de al menos 6 caracteres.")
        isValid= false
      }
